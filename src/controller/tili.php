...
function lisaaTili($formdata, $baseurl='') {
  ...
  // Tarkistetaan, että jäsennumero on määritelty ja se on
  // muodossa 0000.
  if (!isset($formdata['jasennro']) || !$formdata['jasennro']) {
    $error['jasennro'] = "Anna jäsennumerosi muodossa 0000.";
  } else {
    if (!preg_match("/^[0-9]*$/",$formdata['jasennro'])) {
      $error['jasennro'] = "Jäsennumerosi muoto on virheellinen.";
    }
  }
